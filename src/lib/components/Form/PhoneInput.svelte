	...
	const mask = (event) => {
		setTimeout(() => {
			...
			if (event.target.value) {
				...
				for (let targetValueIndex = 0; targetValueIndex < targetValue?.length; targetValueIndex++) {
...
<input
	...
	class="border-0 px-2 h-9 placeholder:font-normal placeholder:text-sm  shadow placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-1 focus:outline-secondary w-full {customClass} disabled:bg-slate-100"
	type="text"
	bind:value
	{placeholder}
