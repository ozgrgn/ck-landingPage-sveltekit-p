<script>
	...
	const addLandingPage = async () => {
		let data = {};
		values.map((v) => {
			data[v.key] = landingPage[v.key]?.value;
		});

		let response = await RestService.addLandingPage(data);
		if (response['status']) {
	...
	values.map((v) => {
		if (v.defaultValue) {
			landingPage[v.key] = { value: v.defaultValue };
		} else {
			landingPage[v.key] = {};
		}
	});
		...
		>
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="text-center flex justify-between">
					<h3 class="font-semibold text-md text-black/70 uppercase ">{page}</h3>
				</div>
			</div>
						...
						<div class="w-full lg:w-6/12 px-4 ">
							<div class="relative w-full mb-3">
								...
								<TextEditor
									...
									bind:isValid={landingPage.infoText2.value}
						...
						<div class="w-full lg:w-6/12 px-4 ">
							<div class="relative w-full mb-3">
								...
								<Input
									...
									placeholder={'(Reviews From OurPatients)'}
									required={false}
								/>
							</div>
						</div>
