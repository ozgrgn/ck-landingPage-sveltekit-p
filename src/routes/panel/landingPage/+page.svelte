<script>
	...
	const getLandingPages = async () => {
		let getLandingPagesResponse = await RestService.getLandingPages(undefined, limit, skip);
		if (getLandingPagesResponse['status']) {
			console.log(getLandingPagesResponse, 'getLandingPagesResponse');
			landingPages = getLandingPagesResponse['landingPages'];
			totalDataCount = getLandingPagesResponse['count'];
		} else {
			...
			<div class="block w-full overflow-x-auto">
				{#if landingPages}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each landingPages as landingPage}
								<tr>
									<td
									...
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{landingPage.lpName ? landingPage.lpName : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/landingPage/update/${landingPage._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteLandingPageApprove(landingPage._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
