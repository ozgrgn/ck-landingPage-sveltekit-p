<script>
	...
	const deleteSharedApprove = (sharedId) => {
		modal.set(
			bind(Alert, {
	...
	const updateShared = async () => {
		let editedShared = {};
		values.map((v) => {
			editedShared[v.key] = shared[v.key].value;
		});

		let response = await RestService.updateShared(shared._id, editedShared);
		if (response['status']) {
	...
	const getShared = async () => {
		let response = await RestService.getShared(data.sharedId);

		if (response['status']) {
			values.map((v) => {
				if (v.customValue) {
					response['shared'][v.key] = {
						value: response['shared'][v.key][v.customValue]
					};
				} else {
					response['shared'][v.key] = { value: response['shared'][v.key] };
				}
			});
			shared = {
				...response['shared']
	...
	const deleteShared = async (sharedId) => {
		let response = await RestService.deleteShared(sharedId);
		if (response['status']) {
			ToastService.success('İşlem başarılı');
			navigate('/panel/shared');
		} else {
			ToastService.success('İşlem başarılı');
		}
	};
	...
	<div class="w-full mb-12 px-2 lg:px-4 ">
		<div class="flex justify-between">
			...
			<button
				class="bg-white text-blue-600 hover:text-red-700 mb-2 border rounded font-bold text-xs px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 "
				type="button"
				on:click={() => deleteSharedApprove(data.sharedId)}
			>
				<i class="fa fa-trash" />
				Sil
			</button>
					...
					<div class="w-full lg:w-12/12 px-4">
						<div class="relative w-full mb-3">
							...
							<TextEditor
								...
								bind:incomingValue={shared.cookies.value}
