<script>
	...
	let shareds;
	const getShareds = async () => {
		let getSharedsResponse = await RestService.getShareds(undefined, limit, skip);
		if (getSharedsResponse['status']) {
			console.log(getSharedsResponse, 'getSharedsResponse');
			shareds = getSharedsResponse['shareds'];
			totalDataCount = getSharedsResponse['count'];
		} else {
								...
								<tr>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/shared/update/${shared._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteSharedApprove(shared._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
